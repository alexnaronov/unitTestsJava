        ...
        if(sf == JFileChooser.APPROVE_OPTION){
           ...
           { 
        	   ...
        	   saveFile.setSelectedFile(new File(saveFile.getSelectedFile().toString() + ext));
           		try {
           			BufferedImage bi = new BufferedImage(Main.frame.getSize().width, Main.frame.getSize().height, BufferedImage.TYPE_INT_ARGB);
           			GraphicAdapter ig2 = new GraphicAdapter() {
           			};
           			ig2.setGraphicAdapter(bi.createGraphics());
           			ig2.getGraphicAdapter().setColor(Color.white);
    				ig2.getGraphicAdapter().fillRect(0, 0, Main.frame.getWidth(), Main.frame.getHeight());
        	   ...
        	   try{
                   ...
                   for(int i = 0; i<Main.paint.size(); i++)
                   {
